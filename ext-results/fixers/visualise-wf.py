...
def plot_task_timeline(tasks):
    # Group tasks by hostname
    tasks_by_node = {}
    for task in tasks:
        node = task["hostname"]
        tasks_by_node.setdefault(node, []).append(task)

    # Sort tasks in each node by start time
    for node_tasks in tasks_by_node.values():
        node_tasks.sort(key=lambda t: t["start"])

    # Compute earliest and latest timestamps for the workflow
    ...
    earliest_hh = int(pd.to_datetime(earliest, unit="ms").round('60min').timestamp() * 1000)
    latest_hh = int(pd.to_datetime(latest, unit="ms").round('60min').timestamp() * 1000)
    diff = 60 * 60 * 1000  # 1 hour in ms
    ...
    # Row assignment to avoid overlap
    current_y = 0
    node_y_positions = {}
    for node, node_tasks in tasks_by_node.items():
        rows = []  # track ending times for each row
        ...
        # Middle y of node for label
        node_y_positions[node] = current_y + (len(rows) - 1) / 2
        current_y += len(rows) + 2  # add extra gap between nodes

    # Plot the tasks
    for task in tasks:
        width = int(task["complete"]) - int(task["start"])
        ax.barh(task["_y"], width, left=int(task["start"]), alpha=0.7)

    # Plot vertical hourly lines
    i = earliest_hh
    while i <= latest_hh:
    ...
    ax.invert_yaxis()
    ax.set_yticks([node_y_positions[n] for n in tasks_by_node.keys()])
    ax.set_yticklabels(list(tasks_by_node.keys()))
    ax.set_xticks(ticks)
    ...
    return get_tasks_by_hour(earliest_hh, latest_hh, tasks)
...
def get_node_idle_periods(tasks):
    """
    Return a dictionary mapping each node to a list of (idle_start, idle_end)
    intervals during which no tasks were running. The overall interval checked
    spans from the earliest task start to the latest task completion.
    """
    # Group tasks by hostname
    tasks_by_node = {}
    for task in tasks:
    ...
    for node, node_tasks in tasks_by_node.items():
        ...
        node_tasks.sort(key=lambda t: int(t["start"]))
        ...
        for task in node_tasks:
            start = int(task["start"])
            end = int(task["complete"])
            # If there's a gap between current and this task's start
            if start > current:
                node_idle.append((current, start))
            # Move current to end if task extends it
            current = max(current, end)

        # If there's remaining time after the last task
        if current < overall_end:
            node_idle.append((current, overall_end))

        idle_periods[node] = node_idle
...
if __name__ == '__main__':
    ...
    idle_times = get_node_idle_periods(tasks)
    export_periods_to_csv(idle_times, "idle")
